...
const Formlivreur =()=>{
    ...
    function validerSaisies() {
      ...
      if (!prenom || !nom || !ville || !region || !adresse || !password || !c_password || !tele) {
        setMessageErreur("Veuillez remplir tous les champs");
        return false;
      } else if (tele.length!=10) {
        setMessageErreur("Veuillez entrer un numéro de téléphone à 10 chiffres");
        return false;
    ...
    <div className="  bg-white/90 rounded-lg lg:w-3/6 mx-auto pt-4  ">
        ...
        <div  className="flex flex-row justify-between gap-8 p-4 lg:p-5">
         ...
         <div  className="flex flex-row justify-between gap-8 p-4 lg:p-5">
                ...
                <div class="w-52 relative before:absolute before:bottom-0 before:h-0.5 before:left-0 before:origin-right focus-within:before:origin-left before:right-0 before:scale-x-0 before:m-auto before:bg-sky-400 dark:before:bg-sky-800 focus-within:before:!scale-x-100 focus-within:invalid:before:bg-red-400 before:transition before:duration-300">
                    <input value={adresse} onChange={(e)=>setadresse(e.target.value)} type="text" placeholder="Adresse " class="w-full bg-transparent pb-3  border-b border-gray-300 dark:placeholder-gray-300 dark:border-gray-600 outline-none  invalid:border-red-400 transition" />
                </div>
                
                <div class="w-52 relative before:absolute before:bottom-0 before:h-0.5 before:left-0 before:origin-right focus-within:before:origin-left before:right-0 before:scale-x-0 before:m-auto before:bg-sky-400 dark:before:bg-sky-800 focus-within:before:!scale-x-100 focus-within:invalid:before:bg-red-400 before:transition before:duration-300">
                    <input value={tele} onChange={(e)=>settele(e.target.value)} type="number" placeholder="Télé" class="w-full bg-transparent pb-3  border-b border-gray-300 dark:placeholder-gray-300 dark:border-gray-600 outline-none  invalid:border-red-400 transition" />
                </div>
            </div>
      ...
      <label className="px-3 text-gray-500">
        <input 
          ...
          onChange={()=>setIsCarSelected(!isCarSelected)} 
          
        />
        voiture
      </label>
      <br></br>
      <label className="px-4 text-gray-600">
        <input 
          type="checkbox" 
          checked={isMotorSelected} 
          onChange={()=>setIsMotorSelected(!isMotorSelected)} 
        />
        Motor
      </label>
